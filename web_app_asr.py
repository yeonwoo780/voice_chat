...
if len(audio) > 0:
    ...
    with st.spinner():
        asr, embed = inference(audio, model_name, model, processor)

    # Display user message in chat message container
    st.chat_message("user").markdown('\n\n'.join([asr, embed]) if voice_embed else asr, unsafe_allow_html=True)
    # Add user message to chat history
    st.session_state.messages.append({"role": "user", "content": asr, "embed": embed})
    ...
    # st.session_state.messages.append({"role": "assistant", "content": asr})
    st.rerun()